<?php
/**
 * §04 — Process. Editable via the front page editor.
 *
 * @package fde-usachim
 */
...
$fde_meta  = (string) fde_field( 'process_meta', 'MIN 0w · TYP 10w · MAX 26w' );
$fde_aside = (string) fde_field(
	'process_aside',
...
foreach ( [ 1, 2, 3, 4, 5 ] as $n ) {
	$i = $n - 1;
	$fde_steps[] = [
		'no'    => (string) fde_field( "process_step_{$n}_no",    $fde_step_defaults[ $i ]['no'] ),
		'dur'   => (string) fde_field( "process_step_{$n}_dur",   $fde_step_defaults[ $i ]['dur'] ),
		'title' => (string) fde_field( "process_step_{$n}_title", $fde_step_defaults[ $i ]['title'] ),
		'desc'  => (string) fde_field( "process_step_{$n}_desc",  $fde_step_defaults[ $i ]['desc'] ),
	];
}
?>
